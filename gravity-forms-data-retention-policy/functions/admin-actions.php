 ...
 */
function gfdrp_verify_admin_action( $action ) {
...
function gfdrp_handle_activate_policy() {
	gfdrp_verify_admin_action( 'gfdrp_activate_policy' );
	$report         = get_transient( gfdrp_report_key( 'policy_report' ) );
	$current_policy = gfdrp_get_site_policy();

	if ( ! is_array( $report ) || ! empty( $report['errors'] ) || ( $report['target_policy'] ?? null ) !== $current_policy ) {
	...
	$result           = gfdrp_synchronize_existing_forms( $previous_policy, $current_policy );
...
function gfdrp_handle_deactivate_unused_forms() {
	gfdrp_verify_admin_action( 'gfdrp_deactivate_unused_forms' );

	$report = get_transient( gfdrp_report_key( 'unused_report' ) );

	if ( ! is_array( $report ) || ! empty( $report['error'] ) || empty( $report['forms'] ) ) {
		wp_safe_redirect( gfdrp_settings_url( array( 'gfdrp_result' => 'unused_test_required' ) ) );
		exit;
	}

	$result = gfdrp_deactivate_unused_forms();
...
function gfdrp_get_unused_forms_report_html( $report ) {
	...
	$html = '<h4>' . esc_html__( 'Active forms with no detected embed', 'gravity-forms-data-retention-policy' ) . '</h4>';
	...
	if ( empty( $report['forms'] ) ) {
		return $html . '<p>' . esc_html__( 'Every active form has a detected reference.', 'gravity-forms-data-retention-policy' ) . '</p>';
	}

	$html .= '<ul>';

	foreach ( $report['forms'] as $form ) {
		$html .= '<li>' . esc_html( '#' . $form['id'] . ' ' . $form['title'] ) . '</li>';
	}
...
function gfdrp_get_policy_controls_html() {
	...
	$report_valid   = $report_matches && empty( $policy_report['errors'] );
	...
	if ( $report_valid ) {
		$html .= '<a class="button button-primary" href="' . esc_url( gfdrp_admin_action_url( 'gfdrp_activate_policy' ) ) . '">' . esc_html__( 'Activate Tested Policy', 'gravity-forms-data-retention-policy' ) . '</a> ';
	}
	...
	$html .= '<p><a class="button" href="' . esc_url( gfdrp_admin_action_url( 'gfdrp_test_unused_forms' ) ) . '">' . esc_html__( 'Scan Active Forms', 'gravity-forms-data-retention-policy' ) . '</a> ';

	if ( is_array( $unused_report ) && empty( $unused_report['error'] ) && ! empty( $unused_report['forms'] ) ) {
		$html .= '<a class="button" href="' . esc_url( gfdrp_admin_action_url( 'gfdrp_deactivate_unused_forms' ) ) . '">' . esc_html__( 'Deactivate Listed Forms', 'gravity-forms-data-retention-policy' ) . '</a>';
	}

	$html .= '</p>' . gfdrp_get_unused_forms_report_html( $unused_report );
